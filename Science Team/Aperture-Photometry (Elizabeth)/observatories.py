...
def get_observer_location(name):
    """
    Given the name of an observatory, return an astropy.coordinates.EarthLocation object.
    Raises ValueError if not found.
    """
    from astropy.coordinates import EarthLocation
    for obs in observatories:
        if obs["name"].lower() == name.lower():
            lat = obs["latitude"]
            lon = obs["longitude"]
            # Default elevation to 0 if not present
            elevation = obs.get("elevation", 0)
            return EarthLocation(lat=lat, lon=lon, height=elevation)
    raise ValueError(f"Observatory '{name}' not found in observatories list.")
